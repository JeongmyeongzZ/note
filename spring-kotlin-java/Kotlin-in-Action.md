...
nullable type 타입이름 뒤에 물음표를 명시하면 된다. String, String? 은 엄연히 달라서 String? 타입은 String 타입에서 제공하는 함수들을 직접 호출할 수 없다.
val x: String? 을 val y = String 에도 대입할 수 없다.

NPE 을 방어할 수 있는 좋은 기회는 생겼지만, 매번 if (x != null) 과 같이 불편하게 검증해야할까?
안전한 호출 연산자 ?. 를 사용하면 된다.

x?.bar() 는 x != null 일 때 x.bar(), x == null 일 때는 null 을 반환한다. 

또 하나의 좋은 장치는 엘비스 연산자 ?: 이다.

company?.address ?: "unknown" 와 같이 예쁘게 코드를 줄일 수 잇다.

NPE 와 비슷하게, 캐스팅에도 ClassCaseException 이 발생할 수 있다. 이 때도 안전한 캐스트 as? 를 지원한다.
foo as? Type 은 foo is Type => foo as Type 을, foo !is Type => null 을 반환한다.

널 아님 단언 (not null assertion) !! 은 널이 아니라고 단언 하는 연산자이다. 물론 그럼에도 불구하고 널인경우 NPE 가 발생한다.
코틀린도 완전히 NPE 에 안전한 언어는 아닌 이유..

여담으로, !! 가 무례해 보일 수 있다. !! 기호는 마치 컴파일러에 소리를 지르는 것 같은 느낌이 든다고 하는데 이걸 의도했다고 한다...
컴파일러에게 검증할 수 없는 걸 단언하기 보다는 더 나은 방법을 찾아보라는 의미가 담겨있다고.. (못생겼으니 다시 한 번 생각해보라는 말도 있다.)

let 함수는 널이 아닌 인자로 함수를 호출할 수 있다. 내장 함수가 아니라 커스터함 함수나, 직접 식을 만들어 사용할 때..

젯브레인스에서는 자바의 @Nullable , @NotNull 과 타입의 조합인 녀석들은 널러블한 타입으로 이해할 수 있다.
...
컴파일러는 fail 로 간 경우 함수가 정상적으로 끝나지 않는다는 것을 명시했기에, address 는 null 이 아니라고 추론이 가능하다. !
